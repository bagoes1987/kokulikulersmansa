...
// Butuh service key untuk admin operations (createUser/updateUser)
...
// Data Fasilitator dari Draf_Akun_Fasilitator.md
const fasilitators = [
    ...
    { no: 37, kelas: "XI. 8", nama: "Wely Marisa, S.Pd.", username: "fasil_wely" },
    { no: 38, kelas: "XI. 9", nama: "A.Thoriq Ganda, S.Pd.", username: "fasil_athoriq" },
    { no: 39, kelas: "XI. 10", nama: "Ari Yani A M, S.T.", username: "fasil_ariyani" },
    { no: 40, kelas: "XI. 11", nama: "Bagus Panca Wiratama, S.Pd., M.Pd.", username: "fasil_bagus" },
    { no: 102, kelas: "XI. 8", nama: "Arif Alimudin Rasyid, S.Pd.", username: "fasil_arif" },
    { no: 103, kelas: "XI. 9", nama: "Rizki Rahmadika, S.Pd.", username: "fasil_rizki" },
    { no: 104, kelas: "XI. 10", nama: "Adventus, S.Pd.", username: "fasil_adventus" },
    { no: 105, kelas: "XI. 11", nama: "Samijo, S.Pd.", username: "fasil_samijo" },
...
async function fixAccounts() {
    console.log("🚀 Memulai perbaikan akun fasilitator...");
    ...
    for (const fasil of fasilitators) {
        try {
            ...
            console.log(`Checking ${fasil.username} (${email})...`);

            // Check if user exists by listing users (simplest way without error)
            // Or just try to create, and if it fails, try to update.

            // Strategy: Try to create. If error says "Email address already registered", then Update.
            ...
            if (createError) {
                if (createError.message.includes("Email address already registered") || createError.status === 422) {
                    console.log(`⚠️ User exists, updating password for ${fasil.username}...`);

                    // Need to find user ID first to update
                    // Can't search by email directly with admin-api in all versions easily, 
                    // but let's try listed listUsers or sign in to get ID? 
                    ...
                    if (user) {
                        ...
                        if (updateError) console.error(`❌ Gagal update ${fasil.username}:`, updateError.message);
                        else console.log(`✅ Berhasil update password ${fasil.username}`);
                    } else {
                        console.error(`❌ User ${email} tidak ditemukan di list (aneh).`);
                    }

                } else {
                    console.error(`❌ Gagal membuat ${fasil.username}:`, createError.message);
                }
            } else {
                console.log(`✅ Berhasil membuat akun baru untuk ${fasil.username}`);
            }
        } catch (err) {
            console.error(`ERROR PROCESSING ${fasil.username}:`, err);
        }
    }
